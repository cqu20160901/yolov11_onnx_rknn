...
def postprocess(output, img_h, img_w):
    print('postprocess ... ')
    output = output[0]
    ...
    for i in range(coord_index):
        for j in range(4, len(CLASSES) + 4, 1):
            if output[coord_index * j + i] > objectThresh:
                classId = j - 4
                score = output[coord_index * j + i]

                cx = output[coord_index * 0 + i]
                cy = output[coord_index * 1 + i]
                cw = output[coord_index * 2 + i]
                ch = output[coord_index * 3 + i]

                xmin = (cx - cw * 0.5) * scale_w
                ymin = (cy - ch * 0.5) * scale_h
                xmax = (cx + cw * 0.5) * scale_w
                ymax = (cy + ch * 0.5) * scale_h
                detectResult.append(DetectBox(classId, score, xmin, ymin, xmax, ymax))
